...
import { FormGroup, FormBuilder, FormGroupName, FormControl } from '@angular/forms';
...
export class CheckoutComponent {
  ...
  onSubmit() {
    ...
    this.checkoutSerive.placeOrder(purchase).subscribe({
      next: response => {
        alert(`Your order has been received. \n Order trakcing number:${response.orderTrackingNumber}`);

        // reset cart
        this.resetCart();
      },
      error: err => {
        alert(`There was an errorL ${err.message}`);
      }
    });
  }
